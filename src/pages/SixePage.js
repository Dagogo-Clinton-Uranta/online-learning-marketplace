   ...
   Mes cours 
     ...
     {packs && packs.map((pack,i)=>(    /*you may need to change the action to fetch subjects inside the pack */
         ...
         style={{ display: 'flex', justifyContent: 'center',marginTop:"20px"}}>
          <SampleCardPage 
          uid={pack.uid} title={pack.title} image = {pack && pack.subjectImageUrl && pack.subjectImageUrl.length > 1?pack.subjectImageUrl:(oldTopics[i] && oldTopics[i].image?oldTopics[i].image:a10)} author ={pack.instructor} price={pack.price} lessons={pack.subjectsInPack.length} time={"2H 26 MINS"} subjectsInPack={pack.subjectsInPack} /> 
     ...
     {topics.map((topic,i)=>(   
         ...
         style={{ display: 'flex', justifyContent: 'center' ,marginBottom:"20px",marginTop:"20px"}}>
          <SmallerCardPage 
          uid={topic.uid}  title={topic.title} image = {topic && topic.subjectImageUrl && topic.subjectImageUrl.length > 1 ?topic.subjectImageUrl:(oldTopics[i] && oldTopics[i].image?oldTopics[i].image:a10)} author ={topic.instructor} price={topic && topic.price} lessons={15} time={"2H 26 MINS"} /> 
         </Grid>
      ))}
