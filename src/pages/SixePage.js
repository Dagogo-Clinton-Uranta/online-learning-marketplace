  ...
  My Courses
     ...
     {topics.slice(0,1).map((topic,i)=>(   
         ...
         style={{ display: 'flex', justifyContent: 'center',marginTop:"20px"}}>
          <SampleCardPage 
          uid={topic.uid} title={topic.title} image = {topic && topic.subjectImageUrl?topic.subjectImageUrl:(oldTopics[i]?oldTopics[i].image:a10)} author ={topic.instructor} price={topic.price} lessons={15} time={"2H 26 MINS"} /> 
     ...
     {topics.slice(1,topics.length).map((topic,i)=>(   
         ...
         style={{ display: 'flex', justifyContent: 'center' ,marginBottom:"20px",marginTop:"20px"}}>
          <SmallerCardPage 
          uid={topic.uid}  title={topic.title} image = {topic && topic.subjectImageUrl ?topic.subjectImageUrl:(oldTopics[i]?oldTopics[i].image:a10)} author ={topic.instructor} price={"22,000"} lessons={15} time={"2H 26 MINS"} /> 
         </Grid>
      ))}
